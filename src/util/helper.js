...
export const getContractError = (err) => {
  ...
  // Debug log to see the actual error
  console.error("Raw error:", err);
  console.error("Error string:", errorStr);

  // Try to extract abort code
  const parts = errorStr.split("Abort Code: ");
  const abortCode = parts.length > 1 ? parseInt(parts[1]) : null;

  console.error("Parsed abort code:", abortCode);
  ...
  return (
    errors[abortCode] ?? errorStr ?? "Transaction failed. Please try again."
  );
};
